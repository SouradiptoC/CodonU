...
def _print_msg(_type: str, output):
    """
    ...
    if _type == 'nuc':
        ...
        print('        - 0: The usage is completely random', file=output)
        print('        - 1: The usage is highly biased', file=output)
...
def generate_report_summary(handle: str, _type: str, genetic_code_num: int, min_len_threshold: int,
    ...
    **NOTE** Possible types are
    ...
    report_file_name = f'summary_report_{identifier}_nuc.txt' if _type == 'nuc' else f'summary_report_{identifier}_aa.txt'
    ...
    if not is_file(report_file_path) or is_file_empty(report_file_path):
        if _type == 'nuc':
            ...
            enc_val = calculate_enc(handle, genetic_code_num, min_len_threshold, gene_analysis)
            with open(report_file_path, 'w') as output:
                ...
                for codon, val in cai_dict.items():
                    print(f'    {codon}: {val}', file=output)
                print('\nCBI Details', file=output)
                for aa, val in cbi_dict.items():
                    print(f'    {seq3(aa)}: {val[0]} (Most used codon {val[1]})', file=output)
                print('\nENc Details', file=output)
                print(f'    ENc Value: {enc_val}', file=output)
        elif _type == 'aa':
            print('Calculating GRAVY score, please be patient, this may take some time.')
            gravy_val = calculate_gravy(handle, min_len_threshold, gene_analysis)
            print('Calculating aromaticity, please be patient, this may take some time.')
            aroma_val = calculate_aromaticity(handle, min_len_threshold, gene_analysis)
            with open(report_file_path, 'w') as output:
                _print_msg(_type, output)
                print('\nGRAVY Details', file=output)
                print(f'    Total GRAVY Value: {gravy_val}', file=output)
                print('\nAromaticity Details', file=output)
                print(f'    Total Aromaticity Value: {aroma_val}', file=output)
