...
def plot_ca_codon_freq_gene(handle: str, genetic_table_num: int, min_len_threshold: int = 200, n_components: int = 2,
    ...
    plt.scatter(x=x, y=y, alpha=0.8, s=10, c=gene_lengths, cmap='viridis', zorder=2)
    ...
    sup_title = f'CA of Codon Frequency of {organism_name} [genes]' if organism_name \
        else 'CA of Codon Frequency [genes]'
    plt.suptitle(sup_title)

    if save_image:
        make_dir(folder_path)
        name = f'CA_codon_freq_gene_{organism_name}.png' if organism_name else 'CA_codon_freq_gene.png'
